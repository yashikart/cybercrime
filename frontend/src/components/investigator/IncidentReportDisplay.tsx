...
function MoneyFlowGraph({ graphData, wallet }: { graphData: Array<{ from: string; to: string; amount: number }>, wallet: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-10 px-3 py-1 bg-emerald-950/60 border border-emerald-500/40 rounded text-xs text-emerald-400 font-mono">
        {Math.round(zoom * 100)}%
      </div>

      {/* Pan Hint */}
      {zoom > 1 && (
        <div className="absolute bottom-4 left-4 z-10 px-3 py-1 bg-gray-900/80 border border-gray-600/40 rounded text-xs text-gray-400 font-mono">
          Click & drag to pan
        </div>
      )}
      ...
      >
        ...
        {nodeList.map((node) => {
          ...
          return (
            <g key={node.id}>
              <circle cx={x} cy={y} r={radius} fill={getNodeColor()} opacity={0.8} />
              <text
                x={x}
                y={y + radius + 15}
                textAnchor="middle"
